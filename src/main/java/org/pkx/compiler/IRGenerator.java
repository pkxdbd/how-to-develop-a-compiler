...
class IRGenerator implements ASTVisitor<Void, Expr> {
    ...
    private org.pkx.asm.Type asmType(Type t) {
        if (t.isVoid()) return int_t();
        return org.pkx.asm.Type.get(t.size());
    }

    private org.pkx.asm.Type varType(Type t) {
        if (! t.isScalar()) {
            return null;
        }
        return org.pkx.asm.Type.get(t.size());
    ...
    private org.pkx.asm.Type int_t() {
        return org.pkx.asm.Type.get((int)typeTable.intSize());
    }

    private org.pkx.asm.Type size_t() {
        return org.pkx.asm.Type.get((int)typeTable.longSize());
    }

    private org.pkx.asm.Type ptr_t() {
        return org.pkx.asm.Type.get((int)typeTable.pointerSize());
    }

    private org.pkx.asm.Type ptrdiff_t() {
        return org.pkx.asm.Type.get((int)typeTable.longSize());
    }
